...
export default function TopNavBar() {
  ...
  // Handles "Hire Me" click:
  // - If already on /contact, smooth-scroll to #contact section
  // - Otherwise, navigate to /contact route
  const handleHireMe = () => {
      ...
      <div className="flex justify-between items-center px-4 md:px-8 py-4 max-w-[1280px] mx-auto relative">
        ...
        <div className={`${isMobileMenuOpen ? "flex" : "hidden"} absolute top-full left-0 w-full bg-surface-container border-b border-white/5 flex-col p-6 gap-6 md:static md:w-auto md:bg-transparent md:border-none md:flex md:flex-row md:p-0 md:gap-8 items-center`}>
          ...
          <Button
            ...
            className="md:hidden w-full bg-secondary-container text-on-secondary-container font-label-md px-6 py-4 rounded-xl hover:bg-secondary-fixed transition-all duration-300 border-0 h-auto"
          >
            {t("nav.hireMe")}
          </Button>
        </div>
        ...
        <Button
          onClick={handleHireMe}
          className="hidden md:block bg-secondary-container text-on-secondary-container font-label-md px-6 py-2 rounded-xl hover:bg-secondary-fixed transition-all duration-300 blue-glow active:scale-95 border-0"
        >
          {t("nav.hireMe")}
        </Button>
      </div>
